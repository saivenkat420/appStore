...
class AppStore extends Component {
  state = {selectedTabId: tabsList[0].tabId, searchInput: ''}
  ...
  render() {
    ...
    let selectedTabApps = appsList.filter(
      eachItem => eachItem.category === selectedTabId,
    )
    selectedTabApps = selectedTabApps.filter(eachItem => {
      const name = eachItem.appName.toLowerCase()
      const input = searchInput.toLowerCase()
      return name.includes(input)
    })
    return (
      <div className="container">
        <h1 className="container-head">App Store</h1>
        ...
        <ul className="app-list-container">
          {selectedTabApps.map(eachItem => (
            <AppItem key={eachItem.appId} values={eachItem} />
          ))}
        </ul>
      </div>
    )
